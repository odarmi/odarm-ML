...
def create_df(places):
    ...
    if 'LineString' in df:
        df.drop(['LineString'], axis=1, inplace=True) 
    #df['LineString'] = df['LineString'].apply(lambda x:[d.split(' ') for d in x if d != 'clampToGround']) # for the map line tracking
...
def get_kml_file(month, day, cookie_content, folder):
    ...
    if len(day_file) == 1 :
        day_file = '0' + day_file
    url = 'https://www.google.fr/maps/timeline/kml?authuser=0&pb=!1m8!1m3!1i2018!2i{0}!3i{1}!2m3!1i2018!2i{0}!3i{1}'.format(month_url, day_url)
    print url
...
def full_df(folder):
    ...
    df = df[['Address', 'BeginDate', 'BeginTime', 'Category', 'Distance', 'Duration',
       'EndDate', 'EndTime', 'IndexTime', 'Name', 'WeekDay']]
